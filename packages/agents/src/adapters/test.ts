import type { AgentEvent, AgentDetection, AgentRunRequest } from '@ai-devflow/core';
...
export interface ControllableTestOptions {
   ...
   * 可根据 req.resumeFrom / req.userInput 区分首次执行与恢复。
   */
 ...
 * 仅用于自动化测试协议边界（异常/超时/提问/恢复/取消），不可替代真实 Agent 验收。
 */
export class ControllableTestAdapter implements AgentAdapter {
  ...
  private resolveScript(req: AgentRunRequest): TestEventSpec[] {
    // 恢复运行（带 userInput）时优先读 RESUME 控制变量，便于 E2E 驱动“待沟通->回答->完成”。
    const envVar = req.userInput
